...
它用于沉淀 Agent Engineering 学习路线、每日笔记、课程实验、阶段项目和最终作品集，不与其他业务项目混合维护。公众号稿件可在同一工作区本地维护，但不上传 GitHub。

| 仓库 | 职责 |
|---|---|
| `agent-engineering-roadmap` | Hello-Agents 14 天冲刺、每日 note、当天代码实验、阶段项目和最终多 Agent 系统 |
...
当前项目以 `roadmap/hello-agents-14-day-sprint.md` 为近期执行主线。结构以“每天”为最小工作单元：
...
- `note.md`，当天学习笔记。
- `experiments/`，当天实际遇到或跑过的代码实验、截图实验、失败记录。
- `references.md`，当天课程链接和补充资料。

`article.md` 和 `writing/` 用于本地公众号创作，写作时使用 `khazix-writer`，由 `.gitignore` 排除，不属于 GitHub 提交证据。

代码实验优先放在当天 `experiments/`，只有形成阶段性可维护成果后再整理进 `projects/`。

## 基本原则

- Git 用于记录整个项目的路线、笔记、代码、进度和维护文档；
- 公众号稿件、发布产物和真实账号凭据只保留在本地，不提交到 GitHub；
